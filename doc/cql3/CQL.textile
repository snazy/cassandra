...
h3(#createSequenceStmt). CREATE SEQUENCE

Note: operations with sequences are not idempotent. I.e. each @a_sequence.nextval@ will return a new, unique value. It is the responsibility of the client to deal with sequences in combinations retry policies correctly.

__Syntax:__

bc(syntax)..
<create-sequence-stmt> ::= CREATE SEQUENCE ( IF NOT EXISTS )?
...
The @CREATE SEQUENCE@ statement creates a new sequence. Sequences in Cassandra generate signed 64-bit integers unique in the whole cluster.

Please note that sequences should only be used if there is no other option to generate unique numbers since generating sequences in a distributed system is a very expensive operation.

Behind the scenes the sequences implementation uses light-weight-transactions and strong consistency to ensure uniqueness of the generated numbers.

When a Cassandra node has to generate a number from a sequence for the first time, it needs to allocate a (range) of sequence numbers.

The number of values reserved for a Cassandra node is determined using the @CACHE@ option.

Then the Cassandra node sequence numbers from the allocated range. Sequence numbers generated from the per-node range are always consecutive.

When a per-node range is exhausted, the node allocates a new range.

Sequences in Cassandra do not support a CYCLE option. That means that a sequence with always generate error when there are no more numbers available - i.e. the sequence reached its maximum value.

To retrieve a sequence's next value, use the @nextval()@ built-in native function. The function takes either one or two parameters.

@nextval(sequence_name)@ just takes the sequence. If the @sequence_name@ is not fully qualified, the keyspace of the table of the @SELECT@#selectStmt is searched first and then the logged keyspace (i.e. the one passed to a "@USE@"#useStmt).

Although it is possible to generate strictly increasing values with the default settings (@CACHE 1@), it is highly recommended to use values for @CACHE@ that matches your workload.
 Setting @CACHE@ too low causes high CAS (LWT) contention.
 Setting @CACHE@ too high causes too big gaps in the numbers generated by different nodes.

@INCREMENT BY@ is the increment of the generated numbers. The default increment is @1@.

@MINVALUE@ defines the minimum allowed value in a sequence. If @MINVALUE@ is omitted or @NO MINVALUE@ specified, the minimum value is @-9223372036854775808@.

@MAXVALUE@ defines the maximum allowed value in a sequence. If @MAXVALUE@ is omitted or @NO MAXVALUE@ specified, the maximum value is @9223372036854775807@.

@START WITH@ defines the initial value of a sequence. If @START WITH@ is omitted, it defaults to the minimum value for sequences with positive @INCREMENT BY@ or to the maximum value for sequences with negative @INCREMENT BY@. The value must be in the range of the configured minium and maximum values.

@CACHE@ defines the amount of values that are reserved by Cassandra nodes. Default is @1@.

@CACHE LOCAL@ defines the amount of values from the per-node reserved range that is held in memory. Default is the value of @CACHE@. This value must not exceed the value for @CACHE@.

@SERIAL CONSISTENCY LEVEL@ defines the serial consistency level used to modify the global synchronization table.

@CONSISTENCY LEVEL@ defines the consistency level used to read from the global synchronization table.
...
SELECT nextval('keyspace_name','sequence_name') FROM some_table;

h3(#alterSequenceStmt). ALTER SEQUENCE

__Syntax:__

bc(syntax)..
<create-sequence-stmt> ::= ALTER SEQUENCE ( IF EXISTS )?
                         ( <keyspace> '.' )? <sequencename>
                         ( INCREMENT ( BY )? <integer> )?
                         ( ( MINVALUE <integer> ) | ( NO MINVALUE ) )?
                         ( ( MAXVALUE <integer> ) | ( NO MAXVALUE ) )?
...
The @ALTER SEQUENCE@ statement modifies a sequence created using @CREATE SEQUENCE@.
See "@CREATE SEQUENCE@":#createSequenceStmt for a description of the various options.

Modifying a sequence invalidates all per-node ranges and can cause significant load to the cluster.

h3(#dropSequenceStmt). DROP SEQUENCE

__Syntax:__

bc(syntax)..
<drop-sequence-stmt> ::= DROP SEQUENCE ( IF EXISTS )?
                         ( <keyspace> '.' )? <sequencename>
p.
...
DROP SEQUENCE my_keyspace.my_sequence;

The @DROP SEQUENCE@ statement removes a sequence created using @CREATE SEQUENCE@.
...
* Sequences are now supported through "@CREATE SEQUENCE@":#createSequenceStmt and "@DROP SEQUENCE@":#dropSequenceStmt.
