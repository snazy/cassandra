...
The optional @TRUSTED@ keyword allows to create (or replace) a so called trusted function. An trusted function is not executed in the UDF sandbox. The permission @TRUSTED@ is required to create (or replace) an trusted function.
