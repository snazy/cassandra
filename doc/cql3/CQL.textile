...
<condition> ::= <identifier> <op> <term>
              ...
              | <identifier> '[' <term> ( '..' ( <term> )? )? ']' <op> <term>
              | <identifier> '[' '..' ( <term> )? ']' <op> <term>
              | <identifier> '[' <term> ( '..' ( <term> )? )? ']' IN <in-values>
...
<selector> ::= <identifier>
             | <collection-identified> '[' <collection-element> ']'
             | <collection-identified> '[' (   <collection-from> '..' <collection-to>
                                             | <collection-from> '..'
                                             | '..' <collection-to> ) ']'
...
h4(#collectionSlices). Collection element or slice selection

Selection of an individual element of a collection is possible by appending @[ <element> ]@ to the collection column name. The returned value is the type of the value - i.e. the element type for sets and the value type for maps. Collection element and slice selections are not supported for lists.

bc(sample). 
CREATE TABLE users (username text PRIMARY KEY, attributes map<text, text>);
SELECT attributes['gender'] FROM users WHERE username = 'ursus';

Selection of a slice of a collection is possible by appending @[ <from> .. <to> ]@ to the collection column name. Either the _from_ or _to_ part can be omitted to make the lower resp. upper restriction unbounded. The returned value is the type of the referenced collection.

bc(sample). 
CREATE TABLE sensor_data (ts timestamp PRIMARY KEY, readouts map<int, text>);
SELECT readouts[1..100] FROM users WHERE ts = ?;

_element_, _from_ and _to_ can be all kinds of allowed terms: literals, bind marker, functions.
...
* "@SELECT@":#selectStmt now allows selecting a single element or a slice of elements from maps and sets
* LWT now allow conditions on slices of collection elements
