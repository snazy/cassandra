...
public class LogHandlingExecutionListener implements TestExecutionListener
{
     ...
     */
    private static class DeferredLogbackInit
    {
        private void logContainerStarted(String className)
        {
            ...
            LoggerFactory.getLogger(className).info("\n\n############################################################\n" +
                                                    "\n                 Container started: {} in Gradle test worker {}\n\n\n",
                                                    className,
                                                    testWorker);
        }
        ...
        switch (testIdentifier.getType())
        {
            case CONTAINER:
                if (source instanceof ClassSource)
                {
                    ...
                    String testWorker = System.getProperty("org.gradle.test.worker");
                    LoggerFactory.getLogger(className).info("\n\n########################################################################################################################\n" +
                                                            "\n                 Container skipped: {} in Gradle test worker {}",
                                                            className,
                                                            testWorker);
        ...
        switch (testIdentifier.getType())
        {
            case CONTAINER:
                if (source instanceof ClassSource)
                {
                    ...
                    String testWorker = System.getProperty("org.gradle.test.worker");
                    LoggerFactory.getLogger(className).info("\n\n########################################################################################################################\n" +
                                                            "\n                 Container finished: {} in Gradle test worker {}",
                                                            className,
                                                            testWorker);
