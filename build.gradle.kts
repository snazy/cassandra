 ...
 *
 * Not implemented yet:
 * - release-packages (RPM / DEB) (use https://github.com/nebula-plugins/gradle-ospackage-plugin/wiki ?)
 * - forking in-jvm-dtests
 * - signing release artifacts (tarballs, packages)
 *
...
tasks.named<Test>("testDistributed") {
    extensions.configure<TestRunnerTaskExtension> {
        timeoutSeconds = 360
...
dependencies {
    ...
    testImplementation("org.apache.cassandra:dtest-api:0.0.1")
    ...
    testRuntimeOnly(project(":tools:junitvintage"))
...
val generateGrammarSourceCassandra by tasks.registering(CAntlrTask::class) {
    ...
    outputDirectory = project.file("${antlrTask.outputDirectory}/org/apache/cassandra/cql3")
    maxHeapSize = "1g"
...
tasks.withType<Test>().configureEach {
    // Additional directories for Gradle disttibuted testing
    ...
    doLast {
        copy {
            from(buildDir.resolve("publications/cassandra")) {
                ...
                rename { n: String -> if ("pom-default.xml" == n) jar.get().archiveFileName.get().replace(".jar", ".pom") else n }
            }
            into(buildDir)
        }
    }
    // Probably want both
    finalizedBy(tasks.named("generatePomFileForCassandraParentPublication"))
}
    ...
    doLast {
        copy {
            from(buildDir.resolve("publications/cassandraParent")) {
                include("pom-default.xml")
                rename { n: String -> if ("pom-default.xml" == n) jar.get().archiveFileName.get().replace(".jar", "-parent.pom") else n }
            }
            into(buildDir)
        }
    }
